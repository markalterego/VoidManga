    ...
    try {
        ...
        for (const type of lists) {
            for (const status of type) {
                for (const entry of status) { // going through entries
                    if (entry.includeInMangadexFetch) { // skip filtered entries
                        ...
                        console.log(`-> ${entry.node.title}\n`); // logging MAL entry title
